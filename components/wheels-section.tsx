...
export default function WheelsSection() {
  return (
    <section id="wheels" className="relative py-24 px-4 bg-[#050505] overflow-hidden">
      ...
      {/* Glow */}
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_right,rgba(239,68,68,0.06)_0%,transparent_60%)]" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div>
            ...
            <div className="mt-8 flex items-center gap-3">
              <Zap className="w-5 h-5 text-red-500/60" />
              <h3 className="text-2xl font-sans font-bold text-red-500 tracking-tight">
                WHEELS AUTOSHOW
              </h3>
            </div>

            {/* Specs */}
            <div className="mt-8 space-y-3 border-l border-red-500/20 pl-6">
              ...
              <div className="flex items-center gap-4">
                ...
                <span className="text-xs font-mono text-red-500/70">Operational</span>
              </div>
              <div className="flex items-center gap-4">
            ...
            <div className="mt-10 flex flex-wrap gap-3">
              {["RALLY", "CAR REVEALS", "STUNTS", "TEMPORAL SHIFT"].map((tag) => (
                <span
                  key={tag}
                  className="text-[10px] font-mono text-white/30 tracking-widest border border-white/5 px-4 py-2 hover:border-red-500/30 hover:text-red-500/60 transition-all cursor-default"
                >
                  {tag}
                </span>
              ))}
          ...
          <div className="relative flex items-center justify-center">
            <div className="relative w-full max-w-md aspect-square">
              ...
              <div className="absolute inset-8 rounded-full border border-red-500/5" />

              {/* Center icon */}
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  <Car className="w-16 h-16 text-red-500/30 mx-auto" />
                  <p className="text-6xl font-sans font-bold text-white/5 mt-4 tracking-tighter">2025</p>
                </div>
              </div>

              {/* Decorative dots */}
              <div className="absolute top-1/4 right-0 w-2 h-2 rounded-full bg-red-500/30" />
              <div className="absolute bottom-1/3 left-0 w-2 h-2 rounded-full bg-red-500/20" />
              <div className="absolute top-0 left-1/2 w-1.5 h-1.5 rounded-full bg-red-500/40" />
            </div>
          </div>
        </div>
      </div>
